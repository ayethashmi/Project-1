...
import com.maxdev.glitenetwork.signalstrenhtmeter.*
import com.maxdev.glitenetwork.speedmeter.SpeedTextListener
...
typealias SpeedTextListenerr = (speed: Float) -> CharSequence
...
abstract class SSGauge constructor(
    context: Context,
     ...
     * maybe null.
     */
    var onSpeedChangeListener: SSOnSpeedChangeListener? = null
    /**
     ...
     * maybe null.
     */
    var onSectionChangeListener: SSOnSectionChangeListener? = null
    ...
    private val _sections = mutableListOf<SSSection>()
    /** All sections. */
    val sections : List<SSSection> get() = _sections
    var currentSection: SSSection? = null
        private set
    ...
    private fun init() {
        ...
        _sections.add(SSSection(0f, .6f, 0xFF00FF00.toInt(), speedometerWidth).inGauge(this))
        _sections.add(SSSection(.6f, .87f, 0xFFFFFF00.toInt(), speedometerWidth).inGauge(this))
        _sections.add(SSSection(.87f, 1f, 0xFFFF0000.toInt(), speedometerWidth).inGauge(this))
    ...
    internal fun checkSection(section: SSSection) {
     ...
     */
    protected fun onSectionChangeEvent(previousSection: SSSection?, newSection: SSSection?) {
...
//    }
     ...
     */
    fun addSections(vararg sections: SSSection) {
     ...
     * @throws IllegalArgumentException If [Section.startOffset] or [Section.endOffset] are invalid.
     */
    fun addSections(sections: List<SSSection>) {
        sections.forEach {
     ...
     */
    fun makeSections(numberOfSections: Int, color: Int, style: SSStyle) {
        _sections.forEach { it.clearGauge() }
        _sections.clear()
        var prevPart = 0f
        var part = 1f / numberOfSections
        for (i in 0 until numberOfSections) {
            _sections.add(SSSection(prevPart, part, color, speedometerWidth, style).inGauge(this))
            prevPart = part
     ...
     */
    fun removeSection(section: SSSection?) {
        section?.clearGauge()
     ...
     */
    private fun findSection(): SSSection? {
        _sections.forEach {
