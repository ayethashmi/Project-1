...
class SSSection @JvmOverloads constructor(
    startOffset: Float,
    endOffset: Float,
    color: Int,
    width: Float = 0f,
    style: SSStyle = SSStyle.BUTT,
): Parcelable {

    private var gauge: SSGauge? = null
    ...
    constructor(section: SSSection) : this(section.startOffset, section.endOffset, section.color, section.width, section.style) {
        padding = section.padding
    }

    constructor(parcel: Parcel) : this(parcel.readFloat(), parcel.readFloat(), parcel.readInt(), parcel.readFloat(), parcel.readSerializable() as SSStyle) {
     ...
     */
    internal fun inGauge(gauge: SSGauge): SSSection {
    ...
    companion object CREATOR : Parcelable.Creator<SSSection> {

        override fun createFromParcel(parcel: Parcel): SSSection {
            return SSSection(parcel)
        }

        override fun newArray(size: Int): Array<SSSection?> {
